...
def test_get_authorization(app, redis_connect):
    ...
    # Test case 35: cert_key in Redis
    ...
    # Test case 36: no cert_key in Redis
    with app.test_request_context():
        # Clear Redis
        redis_connect.delete(cert_key)
        redis_connect.delete(entity_id)
    ...
    # Test case 37: entity_id in Redis
    with app.test_request_context():
        # Clear Redis
        redis_connect.delete(cert_key)
        redis_connect.delete(entity_id)
...
def test_set_management_info(app, redis_connect, mocker):
    ...
    # Test case 41: set management info
    with app.test_request_context():
        # Clear Redis
    ...
    # Test case 43: Redis set() error (entity_id already exists)
    with app.test_request_context():
        # Clear Redis
        redis_connect.delete(management_info_key)
        redis_connect.delete(entity_id)
...
def test_get_access_token(app, redis_connect, mocker):
    ...
    # Test case 44: get access token success
    with app.test_request_context():
        # Clear Redis
        redis_connect.delete(cert_key)
        redis_connect.set(cert_key, json.dumps(cert_val))
    ...
    # Test case 45: no cert_key in Redis
    with app.test_request_context():
        # Clear Redis
        redis_connect.delete(cert_key)
        with patch("requests.post") as mockClient:
...
def test_generate_request_body(app):
    ...
    # Test case 52 : generate request body
    with app.test_request_context():
...
def test_generate_signature(app):
    ...
    # Test case 53 : generate signature
    with app.test_request_context():
...
def test_get_task_status(app, redis_connect):
    ...
    # Test case 56 : status is STARTED
    with app.test_request_context():
        # AsyncResultのモックを作成
    ...
    # Test case 57 : status is SUCCESS
    with app.test_request_context():
        # AsyncResultのモックを作成
        mock_async_result = MagicMock()
        mock_async_result.id = 1
    ...
    # Test case 58 : status is FAILURE
    with app.test_request_context():
        # AsyncResultのモックを作成
        mock_async_result = MagicMock()
        mock_async_result.id = 1
    ...
    # Test case 59 : status is REVOKED
    with app.test_request_context():
        # AsyncResultのモックを作成
        mock_async_result = MagicMock()
        mock_async_result.id = 1
    ...
    # Test case 60: exception occurred
    with app.test_request_context():
        # Clear Redis
        redis_connect.delete(target_key)
        redis_connect.delete(error_key)
